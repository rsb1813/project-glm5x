...
> **Status:** The exact GLM-5.2 reference graph, resumable converter, and CUDA expert sublayer boundaries are implemented and tested. The local full checkpoint is materialized as `282` verified `.k3x` artifacts; lazy assembly indexes `59,585` tensors and `19,456` complete experts. A deterministic `1,454,984`-byte `.gxi` now gives C++ direct tensor ownership for all 282 artifacts, and the first official expert lookup matched Python for all `75,497,472` payload bytes and three role SHA-256 digests. This is a metadata/payload ingress gate, not full-model token execution. The best exact full-model decode measured so far is `0.010559` tok/s. The historical best mixed-NVFP4 gate reached `0.014484` tok/s but changed the output token and is rejected as a quality/default path. Stable and adaptive exact hot-bank policies are implemented and default-off. The adaptive full gate filled the 4 GiB device cache, increased hits from the prior stable control's `21` to `27`, and measured `0.013392` versus `0.013321` tok/s, only a `0.53%` single-run difference. This is not a material promotion result, and no 10 tok/s claim exists. The next performance boundary is connecting exact C++ layer execution to this official index, then adding pooled asynchronous sidecar/H2D scheduling.

...
> **Status:** GLM-5.2 shape/manifest, exact layer-10 q-residual/MLA/DSA/MoE reference, a multi-layer CPU reference with final logits/greedy incremental parity, an out-of-core layer-loader boundary with opt-in trunk-layer caching, the first-three-layer dense MLP reference path, and a learned-router-aware raw-BF16 MoE sublayer CUDA boundary are implemented. The reference model now stages bundle tensors directly on CUDA and has CPU-vs-CUDA parity gates. The decoder layer reuses its DSA q-residual in MLA with exact output parity; an RTX 5080 layer-10 attention probe measured a bounded `3.22%` reduction. An experimental sparse DSA Top-K MLA gather path is opt-in and measured `83.2%` lower attention time at context `16,385` with `0.0278%` relative L2 drift; dense masking remains the default. An opt-in Python expert-major reference path is parity-tested but remains experimental; its real four-token direct MoE gain did not carry over to one-token decode or the full layer. Large expert role payloads now use a read-only memory view before tensor decoding, with a tiny-fixture fallback and no semantic change. The C++ deadline path now has a switchable `1..64` expert-load worker pool with per-key in-flight deduplication; eight is the RuntimeSession default and serial mode remains available. No GLM weights are committed and no end-to-end tok/s number is claimed. A resumable local full-checkpoint stream is active; 188 of 282 shard markers are finalized at the latest local observation. Three disjoint workers are running with no final bundle assembly until all ranges finish. Linux correctness and CodeQL passed on the previous public head; a new CI run will verify this worker-pool change.
...
- The reference logits path prepares the FP32 LM head once per model/device and reuses it across decode steps. This is an exact-path allocation reduction; its additional VRAM cost is measured separately and is not hidden as a TPS claim.
...
- The reference benchmark also exposes an experimental `--expert-precision fp8` row-scaled E4M3 path. It is default-off because the real layer-10 probe measured `5.603%` relative L2 drift and no speedup yet; `bf16` remains the correctness path.

...
- The fingerprinted sidecar now supports row-scaled E4M3 FP8 (`.pf8`) as well as INT4 (`.pi4`). On the real four-token layer-10 probe, a fresh FP8 sidecar process measured `4.8204 s` versus BF16 `11.7594 s`, with `5.70%` relative L2 drift; it remains opt-in pending full-model quality validation.

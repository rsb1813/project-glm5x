...
> **Status:** The exact GLM-5.2 reference graph, resumable converter, and bounded CUDA decoder paths are implemented and tested. The local full checkpoint is materialized as `282` verified `.k3x` artifacts; lazy assembly indexes `59,585` tensors and `19,456` complete experts. A deterministic `1,454,984`-byte `.gxi` gives C++ direct tensor ownership for all 282 artifacts. B-0007 now executes official layer 10 from full-layer input through DSA, MLA, both residuals, natural Top-8 MoE, and full-layer output. Its two-token warm median was `475.927 ms` (`237.964 ms/token`) with exact DSA Top-K and routed-expert sets, and `0.4202%` BF16-boundary relative error. Route ordering was not bitwise-identical, so this is a bounded numerical integration gate rather than strict trace parity or full-model token execution. The best exact full-model decode remains `0.010559` tok/s. The historical best mixed-NVFP4 gate reached `0.014484` tok/s but changed the output token and is rejected as a quality/default path. No 10 tok/s claim exists. The next boundary is a production batched/resident multi-layer C++ decoder followed by final-logit parity and asynchronous sidecar/H2D scheduling.

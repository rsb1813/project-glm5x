...
> **Status:** The exact GLM-5.2 reference graph, resumable converter, and CUDA expert sublayer boundaries are implemented and tested. The local full checkpoint is materialized as `282` verified `.k3x` artifacts; lazy assembly indexes `59,585` tensors and `19,456` complete experts. A deterministic `1,454,984`-byte `.gxi` gives C++ direct tensor ownership for all 282 artifacts. B-0006 now feeds the existing learned layer-10 CUDA MoE directly from that index: both the five-artifact control and the official index selected the same 16 routed experts, used the same contributions, read the same `1,286,603,776` bytes, and produced the same GLM5XACT error. This is still one MoE sublayer, not full-model token execution. The best exact full-model decode measured so far is `0.010559` tok/s. The historical best mixed-NVFP4 gate reached `0.014484` tok/s but changed the output token and is rejected as a quality/default path. Stable and adaptive exact hot-bank policies are implemented and default-off. No 10 tok/s claim exists. The next boundary is exact C++ decoder-layer/final-logit integration followed by pooled asynchronous sidecar/H2D scheduling.
...
- C++ `Glm5xRuntimeIndex` now opens the official 282-shard `.gxi`, validates every referenced artifact and locator, and reads one tensor or expert through its exact extent. The B-0005 layer-10 expert-48 gate matched Python byte-for-byte; full decoder-layer/final-logit integration remains open.

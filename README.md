...
> **Status:** GLM-5.2 shape/manifest, exact layer-10 q-residual/MLA/DSA/MoE reference, a multi-layer CPU reference with final logits/greedy incremental parity, an out-of-core layer-loader boundary with opt-in trunk-layer caching, the first-three-layer dense MLP reference path, and a learned-router-aware raw-BF16 MoE sublayer CUDA boundary are implemented. The reference model now stages bundle tensors directly on CUDA and has CPU-vs-CUDA parity gates. The opt-in device-side expert accumulator is parity-tested but remains experimental. No GLM weights are committed and no end-to-end tok/s number is claimed. A resumable local full-checkpoint stream is active; 23 of 282 shards have been converted and their verified source files deleted. Three disjoint workers are running with no final bundle assembly until all ranges finish. The latest public checks for `5c5a2eb` are Linux correctness `31835150475` and CodeQL `31835150428`, both green.

...
> **Status:** The exact GLM-5.2 reference graph, resumable converter, and CUDA expert sublayer boundaries are implemented and tested. The local full checkpoint is materialized as `282` verified `.k3x` artifacts; lazy assembly indexes `59,585` tensors and `19,456` complete experts. The best exact full-model decode measured so far is `0.010559` tok/s. The latest mixed-NVFP4 gate reached `0.014484` tok/s but changed the output token and is rejected as a quality/default path. Stable per-layer hot-bank admission is implemented and default-off. A same-HEAD full-model A/B measured `0.013321` tok/s for stable versus `0.013164` for layer-balanced, only a `1.19%` directional gain, while leaving `1.346 GB` of the 4 GiB cache unused. The next experiment uses that spare budget for repeatedly observed experts before committing to a larger asynchronous replay-prefetch architecture; no 10 tok/s claim exists.
...
- Stable per-layer hot-bank admission that preserves repeatedly used exact experts and bypasses one-off device-cache churn. It is default-off until natural-router full-model traces confirm the bounded result.

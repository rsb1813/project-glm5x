...
> **Status:** The exact GLM-5.2 reference graph, resumable converter, and bounded CUDA decoder paths are implemented and tested. The local full checkpoint is materialized as `282` verified `.k3x` artifacts; lazy assembly indexes `59,585` tensors and `19,456` complete experts. A deterministic `1,454,984`-byte `.gxi` gives C++ direct tensor ownership for all 282 artifacts. B-0009 adds an opt-in resident W8A16-G128 path for natural Top-8 routed experts while retaining BF16 activations, shared expert, and trunk. On the official layer-10 gate it reduced routed payload bytes by `49.22%` and improved the two-token warm median from `5.503 ms` to `4.521 ms` (`17.86%`), with identical route IDs/contributions and zero warm weight H2D. The one-token gate measured `2.223 ms/layer-token`; multiplying that one layer by 78 gives an optimistic `5.767 tok/s` upper bound before final norm, logits, storage, runtime control, and heterogeneous-layer effects. These are layer-only results, not model TPS. The best exact measured full-model decode therefore remains `0.010559` tok/s. The historical best mixed-NVFP4 gate reached `0.014484` tok/s but changed the output token and is rejected as a quality/default path. No 10 tok/s claim exists. The next boundary is device-resident attention/trunk execution, persistent packed W8 artifacts, and multi-layer final-logit parity.

The current low-loss precision decision is explicit: W8A16-G128 is the accepted bounded routed-expert candidate, with BF16 fallback always available. Official FP8 remains an interoperability/comparison artifact, while native Blackwell FP4 remains a more aggressive research path. W8A16 and NVFP4 are both default-off until persistent artifact, multi-layer final-logit, and full-model quality/throughput gates pass.
...
- Stable and adaptive exact hot-bank admission. The adaptive tier preserves one base expert per layer and uses spare VRAM only for second-or-later observations; both policies remain default-off after the full gate showed only a `0.53%` directional decode change.
...
- `glm5x-convert build-runtime-index` emits a deterministic fixed-record `.gxi` for the C++ runtime. Its 128-byte header, artifact table, sorted tensor locators, body SHA-256, header CRC32C, and artifact root bindings provide direct lookup without a general JSON dependency or per-request 282-shard scan.
...
- C++ `Glm5xRuntimeIndex` now opens the official 282-shard `.gxi`, validates every referenced artifact and locator, and reads one tensor or expert through its exact extent. B-0005 matched layer-10 expert 48 byte-for-byte, and B-0007 uses the same source for a complete bounded layer-10 decoder gate; multi-layer/final-logit integration remains open.
- `k3x_cuda_glm5x_real_expert_bench --runtime-index FILE` now uses the same index for router, correction bias, routed experts, and the shared expert. B-0006 preserves the artifact-directory path as a control and reports source artifacts, calls, and bytes separately from CUDA residency/H2D telemetry.
- `--input-mode learned-decoder-layer` extends that bounded runner through official layer-10 input RMSNorm, q-residual, full DSA indexer, MLA attention, both residuals, learned routed/shared MoE, and the full-layer BF16 comparator. B-0007 is explicitly layer-only and does not report full-model decode tok/s.
- Resident raw-BF16 dense hits now reuse the validated device pointer before host-weight conversion. B-0008 proves that the second call converts zero additional weight bytes and preserves output/H2D parity; its `2.757 ms` single-token layer result remains a bounded kernel/runtime gate, not full-model throughput.
- The same official decoder-layer runner now has an opt-in resident W8A16-G128 routed-expert path. B-0009 preserves natural Top-8 routing and BF16 shared/trunk execution, reduces routed payload bytes to `50.78125%` of BF16, and measures `2.223 ms/layer-token` on the one-token layer-10 gate. It remains experimental and default-off; `.pw8` manufacturing and full-model final-logit/TPS evidence are not implemented yet.
...
- The first full 78-layer NVFP4 gate completed with a 40 GiB trunk cache and 1,200 `.pgu` sidecar hits. It measured `0.002231` prefill tok/s, `0.005469` decode tok/s, `631.127 s` TTFT, `33.396 GB` prefill K3X bytes/token, and `0` decode K3X bytes/token. The exact BF16 resident-trunk control measured `0.003236` prefill tok/s, `0.009696` decode tok/s, and `412.119 s` TTFT. NVFP4 generated token `154820` while BF16 generated `565`, so the current mixed FP4 mode is rejected as a default despite the lower measured bundle-read counter; sidecar I/O is not included in that counter.
- NVFP4 gate/up now shares dynamic activation quantization between the two projections. A resident RTX 5080 GLM-shaped microbenchmark measured `0.779 ms` paired versus `1.407 ms` for independent calls (`1.81x`), while a 4 GiB full-model device-cache gate still recorded `0` cache hits and `1,691` evictions. The next cache work is layer-aware protected residency, not plain LRU growth.
...
5. CUDA DSA/MLA, Top-8 MoE, and compressed-KV kernels. (One official index-backed layer is integrated; production batched kernels and persistent multi-layer state remain pending.)

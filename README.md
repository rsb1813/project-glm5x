...
> **Status:** The exact GLM-5.2 reference graph, resumable converter, and bounded CUDA decoder paths are implemented and tested. The local full checkpoint is materialized as `282` verified `.k3x` artifacts; lazy assembly indexes `59,585` tensors and `19,456` complete experts. A deterministic `1,454,984`-byte `.gxi` gives C++ direct tensor ownership for all 282 artifacts. B-0008 removes repeated host BF16 conversion on resident dense-weight hits. The official layer-10 two-token warm median fell from `475.927 ms` to `5.503 ms` (`86.48x`) with unchanged bounded numerical error and zero warm weight H2D; a separate one-token gate measured `2.757 ms/layer-token`. These are layer-only results, not model TPS. Even the optimistic 78-layer multiplication is only `4.651 tok/s` before final norm, logits, storage, and runtime overhead. The best exact measured full-model decode therefore remains `0.010559` tok/s. The historical best mixed-NVFP4 gate reached `0.014484` tok/s but changed the output token and is rejected as a quality/default path. No 10 tok/s claim exists. The next boundary is calibrated native FP4 or a comparably large resident weight-traffic reduction, followed by multi-layer final-logit parity.

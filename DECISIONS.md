...
## D-0099 -- Use spare stable-bank VRAM for repeated experts before async replay prefetch

- Decision: keep `stable_hot_bank` default-off and do not immediately add model-level CUDA replay-prefetch tickets. First add an experimental adaptive exact tier that preserves the one-per-layer base bank while using otherwise idle device-cache bytes only for experts observed at least twice.
- Alternatives: promote the current stable policy, revert to layer-balanced, or proceed directly to previous-token route replay on a dedicated CUDA stream. Promotion is unsupported by a `1.19%` single-pair gain; layer-balanced consumes more VRAM without more hits in the paired run; replay prefetch crosses session state, event lifetime, unused-ticket cleanup, and exact fallback boundaries.
- Evidence: current-HEAD 78-layer A/B produced identical mixed-NVFP4 tokens. Stable measured `0.013321270279236509` decode tok/s, `21/1800` device hits, `2,949,120,600` resident cache bytes, and `11,034,244,608` peak allocated VRAM. Layer-balanced measured `0.013164422849939591`, `18/1800`, `4,286,055,272`, and `12,371,738,112`. Stable left `1,345,846,696` configured cache bytes unused.
- Accepted because: an adaptive repeated-expert tier can increase exact residency without new routing semantics, H2D speculation, or stream ownership. It directly targets measured unused capacity and is independently reversible.
- Revisit: if adaptive admission fails to increase natural-route hits or decode throughput, stop extending Python cache heuristics and design the explicit per-session replay-prefetch ticket architecture with bounded pending bytes and final-token parity.

## D-0100 -- Retain adaptive hot-bank support but stop extending Python cache heuristics

- Decision: keep `adaptive_hot_bank` as an opt-in exact cache policy, but do not promote it and do not add another Python-only admission heuristic. Move the performance path to the official full-model C++ runtime plus pooled asynchronous sidecar/H2D scheduling.
- Alternatives: promote adaptive based on one directional full run, spend another milestone tuning admission thresholds, or jump directly to lossy routing/proxy reduction. The first overstates a `0.53%` cross-run difference, the second cannot remove Python orchestration and synchronous H2D, and the third violates the current quality priority.
- Evidence: B-0004's bounded trace increased steady hits `16→20` and reduced H2D bytes `3.571%`, but warm wall median worsened `4.998%`. The 78-layer adaptive run produced the same mixed-path tokens `[154820,474]`, increased hits `21→27`, and measured `0.013392177383` versus the unchanged ancestor stable control's `0.013321270279` tok/s while adding `608,498,688` peak VRAM bytes. Mixed NVFP4 remains quality-rejected against exact BF16.
- Accepted because: the feature preserves exact selected payloads and exposes useful residency telemetry, but the measured gain is too small and noisy to justify more work at the Python policy layer. The dominant remaining boundary is full-model transport/runtime integration.
- Revisit: after the C++ official-bundle path reports exact final-token/logit parity, physical sidecar bytes, H2D bytes/time, queue stalls, and end-to-end decode improvement with bounded asynchronous ownership.

## D-0101 -- Use a fixed validated `.gxi` as the official C++ shard ingress

- Decision: generate one small deterministic runtime index from the verified JSON bundle and let C++ resolve `tensor_id -> artifact/record` directly. Keep payloads in the existing 282 K3X artifacts and validate artifact roots, tensor counts, locator identities, and CRC metadata when opening the index.
- Alternatives: retain the existing per-role 282-shard scan, add a general JSON parser to the C++ hot path, or merge all payloads into a new monolithic artifact. Repeated scans do not scale to full-layer execution; a general JSON dependency adds unnecessary parsing/state; merging would duplicate roughly checkpoint-scale storage and delay iteration.
- Evidence: B-0005 produced a `1,454,984`-byte index for 282 artifacts and 59,585 tensors in `5.29 s`. The C++ gate opened the official metadata and read `75,497,472` bytes for layer 10 expert 48 in `3.31 s`; all three role SHA-256 digests matched the independent Python bundle read. The MTP regression reproduces official layer-78 non-expert and expert tensors and passes while main `LAYR`/`EXPT` validation remains strict.
- Accepted because: it removes per-request shard discovery, preserves exact existing payload bytes and crash-safe artifacts, adds no general parser dependency, and establishes a bounded C++ entry point for official full-model integration.
- Revisit: after the decoder uses the index for exact trunk and expert tensors. Measure index-open reuse, descriptor residency, direct-I/O/page-cache behavior, H2D bytes, final-logit parity, and end-to-end decode before changing startup validation or mapping policy.

## D-0102 -- Use `.gxi` as the exact source for the existing CUDA MoE gate

- Decision: add a validated metadata-plus-payload read to `Glm5xRuntimeIndex` and let the existing learned-MoE benchmark select either the bounded artifact directory or the official runtime index. Keep all routing, expert-major CUDA, shared-expert, activation, and expected-output logic common.
- Alternatives: build a second index-specific benchmark, add a general tensor-source class hierarchy, or wait for the entire C++ decoder before exercising official indexed weights. A second benchmark would duplicate correctness logic; a class hierarchy is unnecessary for two concrete sources; waiting would leave the new ingress untested by real CUDA execution.
- Evidence: B-0006's two rows selected identical routes/contributions, read 53 extents and `1,286,603,776` bytes, admitted `1,283,457,024` bytes, moved zero warm weight bytes, and produced the same CPU/GPU and GLM5XACT errors. The index path completed host payload loading in `15.602 s` versus `16.467 s` for the control in one pair, but warm CUDA latency was `4.52%` higher, so no speed promotion is inferred.
- Accepted because: the official 282-shard model now reaches an existing exact C++/CUDA execution boundary without Python lookup, payload duplication, routing changes, or a new engine. The artifact-directory control remains available for regression.
- Revisit: after exact q-residual/MLA/DSA and final-logit integration. At that point replace per-invocation payload vectors with bounded resident/pinned ownership and measure physical NVMe, H2D, queue stalls, layer latency, and end-to-end decode.

## D-0103 -- Accept a benchmark-local full decoder layer before generalizing the runtime API

- Decision: extend the existing official CUDA benchmark with one complete layer-10 path sourced from `.gxi`, and accept it only as a bounded BF16 numerical integration gate. Do not yet add a public general decoder-layer API or infer full-model throughput.
- Alternatives: first design a production multi-layer attention interface; orchestrate trunk attention in Python around the C++ MoE; or skip directly to final logits. The first would generalize an unmeasured design, the second would preserve the Python bottleneck, and the third would make layer-local correctness failures harder to isolate.
- Evidence: B-0007 executed DSA, MLA, both residuals, natural Top-8 routed/shared MoE, and the final layer output from the official 282-artifact index. Python and C++ matched DSA Top-K and selected expert sets; maximum contribution delta by expert was `0.000188679`; final BF16-boundary relative error was `0.00420168`.
- Benchmark result: the one-layer two-token warm median was `475.927299 ms`, with `1,632,239,616` resident weight bytes and zero warm weight H2D. Route ordering was not bitwise-identical, so strict trace parity remains open and no full-model tok/s is recorded.
- Reason accepted: this is the smallest complete boundary that proves official trunk, attention, routing, experts, and residual ownership can meet in C++ without Python tensor lookup. It also exposes per-token projection/control overhead before a large production API is frozen.
- Revisit: replace benchmark-local per-token matvec and host normalization/softmax with batched/fused resident execution, preserve or explicitly canonicalize route trace ordering, then require multi-layer state and final-logit parity before promotion.

## D-0104 -- Reuse resident raw-BF16 dense weights before host conversion

- Decision: add an internal resident-key lookup and perform it before converting a complete FP32 host weight view to BF16. Keep activation conversion per call and preserve the existing miss, validation, admission, and transient fallback paths.
- Alternatives: retain the conversion cache keyed by host pointer, move the whole decoder to a new device graph immediately, or quantize the trunk before fixing the exact path. Pointer caching still scans/copies hundreds of megabytes per invocation; a new graph would combine ownership and correctness risks; quantization would obscure the exact BF16 bottleneck.
- Evidence: the CUDA regression shows one conversion on first admission and zero additional conversion bytes on the second resident call, with exact repeated output and unchanged warm H2D. B-0008 reduced the official two-token layer-10 median from `475.927299 ms` to `5.503443 ms` (`86.4781x`) while retaining the same DSA/expert-set boundary and `0.0042016809` expected-output relative error.
- Benchmark result: the independent one-token gate measured `2.756654 ms/layer-token` and zero warm H2D. Its optimistic 78-layer multiplication is `215.019 ms/token` or `4.6508 tok/s` before final norm, logits, I/O, and runtime overhead; this is derived rather than measured full-model TPS.
- Reason accepted: the change removes verified redundant work without changing model arithmetic, routing, storage format, or public behavior. It is independently reversible and its telemetry directly proves the intended effect.
- Revisit: the residual profile is BF16 GEMV-dominated. Evaluate a native calibrated FP4 resident representation or an equivalently large packed/fused projection design, and require multi-layer final-logit plus quality parity before promotion.

## D-0105 -- Accept W8A16-G128 as the bounded low-loss routed-expert candidate

- Decision: add a fixed signed W8A16-G128 resident representation for natural Top-8 routed experts only. Keep BF16 activations, the shared expert, decoder trunk, FP32 accumulation, and the complete BF16 fallback. Keep W8A16 default-off until persistent artifact and multi-layer gates pass.
- Alternatives: all-routed NVFP4, all-routed FP8, mixed BF16/FP8 rescue, W6A16-G32, and heterogeneous projection grouping. NVFP4 exceeded the quality budget, FP8/mixed rescue retained too many BF16 bytes for the resulting error, W6 exceeded one-percent layer L2, and heterogeneous grouping regressed the bounded runtime.
- Evidence: official layer-10 calibration measured W8A16-G128 at `0.8926288%` relative L2 and `50.78125%` of BF16 expert bytes. B-0009 then preserved natural route IDs/contributions, moved zero warm weight bytes, and measured complete-layer GPU/CPU relative L2 of `0.02295%` for two tokens and `0.02331%` for one token.
- Benchmark result: the two-token warm median improved from B-0008 BF16 `5.503443 ms` to `4.520669 ms` (`17.857%` lower), while routed payload bytes fell from `1,207,959,552` to `613,416,960`. The one-token layer measured `2.223067 ms`; its 78-layer multiplication is an optimistic `5.767 tok/s` bound, not measured model throughput.
- Reason accepted: it is the first measured packed routed-expert path to satisfy the one-percent quality gate, preserve exact natural routing, halve routed weight bytes, maintain zero warm H2D, and materially improve the complete bounded decoder layer.
- Rejected experiment: grouping heterogeneous attention/trunk projections increased a bounded sample from approximately `4.198 ms` to `5.420 ms`; the change and its test were removed rather than merged.
- Revisit: implement fingerprint-bound `.pw8` manufacturing without full-model RAM residency, keep scales/outlier metadata versioned, then require multi-layer final-logit/greedy-token parity and measured end-to-end decode. Device-resident attention/trunk execution is the immediate latency bottleneck before the 10 tok/s target can be evaluated honestly.

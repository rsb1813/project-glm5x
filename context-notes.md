...
- Added `GLM5XExpertTensorCache` for optional decoded target-device residency. The same five-shard layer-10 probe with 1,000,000,000 bytes took `3.124653389 s` cold and `0.003793419 s` warm, output max difference `0.0`, 8 device-cache hits, and `719,427,584` bytes peak CUDA allocation. This remains a bounded sublayer result until the full 78-layer gate.
- Extended the full-bundle benchmark JSON with per-decode-step seconds/tok/s and cumulative host/device cache hits so the cold versus warm token boundary is observable rather than hidden behind an aggregate average.

## 2026-08-15 -- CI alert triage and live conversion refresh

...
- Added `tools/monitor_glm5x_full_gate.sh` and started it as a hidden local WSL process. It polls the 282-marker gate, then runs lazy bundle assembly and a one-token CUDA reference benchmark into `build-glm5x-full-stream-final.json` and `build-glm5x-full-reference-cuda.json`; no cloud or paid resource is involved.

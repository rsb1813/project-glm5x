...
- The real-shard path now has an opt-in dense BF16 expert-major grid. It accepts a candidate-token block, admits each expert's three BF16 projections once through `ResidentWeightTable`, runs gate/up/down projections with cublasLt, and returns one flattened output per expert. The benchmark keeps FP32 on the scalar reference path and uses the grid only for BF16 until a direct raw-BF16 FP32-quality path exists.

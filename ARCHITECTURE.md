...
- The Python reference `GLM5XExpertBundle` reopens and identity-checks every referenced artifact, verifies each role's record metadata, and reads exact raw-BF16 bytes by `(layer, expert)` without concatenating payload files.

...
- FP4 direction: the current uncommitted implementation adds native NVFP4 scaled-GEMM and `.pn4`/`.pgu` sidecars. Focused verification is `21 passed`; full-model FP4 quality and throughput are still pending.
...
- Current bottleneck: storage read/reload, not CUDA arithmetic. The exact reference is operational for a correctness baseline, but it is not yet an interactive local engine. NVFP4 is now a real Blackwell kernel/storage experiment, yet its current layer-level drift is not acceptable for a default and it does not reduce the full-model reload bound until connected to multi-layer residency. The next target is calibrated FP4 residual handling plus resident trunk/expert reuse and layer-aware pinned/asynchronous staging, followed by a fresh measured gate.

...
Latest applicable verification at implementation commit `3b7a35a`: CUDA CTest `27/27`, CPU CTest `15/15`, and focused Python `5 passed`. The most recent complete Python regression remains B-0007's `375 passed, 126 skipped`; it was not repeated for the C++-only B-0008 optimization. Historical milestone sections below retain their original counts for audit context.

## Current milestone

The official 282-shard `.gxi` now feeds one complete layer-10 C++/CUDA decoder boundary. B-0008 removes repeated host BF16 conversion on resident dense hits and reduces that boundary from `475.927299 ms` to `5.503443 ms` for two tokens while preserving the bounded numerical gate and zero warm H2D. A separate one-token run measured `2.756654 ms/layer-token`. This is layer-only evidence: the best exact measured full-model decode remains `0.010559 tok/s`, mixed NVFP4 remains quality-rejected, and 10 tok/s is not achieved. The current implementation priority is calibrated native FP4 or an equivalently large packed/fused resident projection path, followed by multi-layer final-logit parity.
...
- Current bottleneck: the bounded resident C++ layer no longer spends its warm path reconverting host weights or moving weights over H2D. Nsight attributes approximately `87.9%` of its remaining aggregate GPU kernel time to BF16 GEMV. The exact Python full-model path remains storage/reload-bound, while the mixed NVFP4 path still has final-token divergence.
...
- Verification: implementation commit `3b7a35a` passed CUDA CTest `27/27`, CPU CTest `15/15`, and focused Python `5 passed`. B-0008 contains separate 100-iteration two-token and single-token raw gates. No full-model speed claim changed.
- GitHub: branch `codex/index-backed-cuda-moe` is published outside the sandbox to the public `rsb1813/project-glm5x` repository. Draft PR [#12](https://github.com/rsb1813/project-glm5x/pull/12) targets `codex/cpp-runtime-index`; commit `3b7a35a` is on the remote branch.

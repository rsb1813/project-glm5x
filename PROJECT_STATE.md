...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, the exact q-residual/MLA/DSA/MoE reference, configuration-driven all-layer loading, resumable local full-checkpoint streaming, lazy final bundle indexing, disjoint shard-range workers, exact cache/prefetch experiments, CUDA expert-major sublayer boundaries, logical storage telemetry, grouped decoder-layer reads, exact FP32 LM-head reuse, and the explicit full-bundle benchmark gate are implemented. The latest locally verified implementation is `6e8c289`; public Linux correctness `31858950666` and CodeQL `31858950671` both passed, C++ CTest is green at `15/15`, and the full Python suite is green at `327 passed, 124 skipped`. All 282 real shards now have verified `.k3x` artifacts and source-deletion markers, and lazy assembly reports 19,456 complete experts. The first exact full-model CUDA gate is measured: cold prefill `0.003258 tok/s`, cold decode `0.003304 tok/s`, TTFT `609.621 s`, and logical storage reads `79,763,152,896` bytes per token. The cached two-token gate remains exact but records `0.003271 tok/s`, `0.0` expert-cache hit rate, and `159,526,305,792` decode bytes because the current 8 GiB host/4 GiB device capacity is smaller than the working set and the trunk is still reloaded. These are baseline measurements, not a target claim. Full CUDA final-logit quality, MTP, paged KV, asynchronous residency, and optimized end-to-end execution remain open.

## Current live snapshot -- 2026-08-15 13:10 KST
...
- GitHub: public head `6e8c289` has successful correctness run `31858950666` and CodeQL run `31858950671`. The recurring red `correctness / Linux (push)` notification is stale run `31795400168` on `b94c8b8`, where 50 historical evidence files were absent; it is not an active failure on `main`.
...
- Public verification before this local-only script repair remains green at `6e8c289`: correctness `31858950666` and CodeQL `31858950671`. The script repair is pending the next intentional push; no cloud or paid resource was used.

...
Latest applicable verification at W8A16 implementation commit `51ca9bc`: CUDA CTest `29/29`, CPU CTest `16/16`, and focused Python contract `6 passed`. B-0009 raw JSON hashes, summary values, routing, warm-H2D, derived formula, and CSV row parity were independently rechecked. The most recent complete Python regression remains B-0007's `375 passed, 126 skipped`; it was not repeated for this C++/CUDA-only bounded optimization. Historical milestone sections below retain their original counts for audit context.

## Current milestone

The official 282-shard `.gxi` feeds one complete layer-10 C++/CUDA decoder boundary. B-0009 replaces only natural Top-8 routed experts with resident W8A16-G128 while retaining BF16 activations, shared expert, trunk, and fallback. The two-token warm median improved from B-0008 BF16 `5.503443 ms` to `4.520669 ms`, routed payload bytes fell `49.21875%`, routes/contributions remained identical, and warm weight H2D remained zero. The one-token gate measured `2.223067 ms/layer-token`. This is layer-only evidence: its optimistic 78-layer multiplication is `5.767 tok/s`, the best exact measured full-model decode remains `0.010559 tok/s`, and 10 tok/s is not achieved. The current implementation priority is device-resident attention/trunk execution, then persistent `.pw8` artifacts and multi-layer final-logit parity.

## Current live snapshot -- 2026-08-16
...
- Current bottleneck: W8 routed projections now sustain roughly `825--837 GB/s` effective weight traffic and reduce the bounded complete-layer median by `17.86%`, so further expert-kernel-only tuning cannot reach 10 tok/s. Attention/trunk execution still crosses host control and launches multiple resident matvecs; the exact Python full-model path remains storage/reload-bound, while mixed NVFP4 still has final-token divergence.
...
- W8A16 bounded result: B-0009 contains 100-iteration two-token and single-token raw gates. Complete-layer GPU/CPU relative L2 was `0.02295%` and `0.02331%`; the path is accepted only as experimental/default-off until persistent `.pw8` and multi-layer quality gates exist.
- Verification: implementation commit `51ca9bc` passed CUDA CTest `29/29`, CPU CTest `16/16`, and focused Python `6 passed`. B-0009 artifact/hash/formula parity passed. No full-model speed claim changed.
- GitHub: branch `codex/index-backed-cuda-moe` is published outside the sandbox to the public `rsb1813/project-glm5x` repository. Draft PR [#12](https://github.com/rsb1813/project-glm5x/pull/12) targets `codex/cpp-runtime-index`; W8A16 evidence commit `fe117c0` is on the remote branch.
...
## 2026-08-16 -- Adaptive exact hot-bank milestone

- Current milestone: `adaptive_hot_bank` is implemented as a separate opt-in decoded-expert cache policy on branch `codex/adaptive-hot-bank`. Implementation commit is `3e0be9f`; B-0004 evidence and synchronized architecture/decision/benchmark documents are in `11a94a0`.
- Completed work: TDD coverage for first-observation bypass, second-observation admission, base protection, and strictly-colder extra replacement; model/CLI validation; configurable bounded benchmark metadata; deterministic real-sidecar A/B; and one 78-layer natural-routing full gate.
- Bounded result: adaptive increased steady exact hits `16→20` and reduced H2D payload `4,404,020,096→4,246,733,664` bytes, but warm median wall time increased `3.268563048→3.431929617 s`. It is not a throughput win.
- Full result: adaptive generated `[154820,474]`, measured `0.013392177383` decode tok/s, 27/1,800 device hits, 34 promotions, 109 resident entries, `4,286,055,272` resident cache bytes, and `11,642,743,296` peak allocated VRAM bytes. The unchanged ancestor stable control measured `0.013321270279` tok/s and 21 hits; the `0.532%` difference is directional and within plausible run noise.
- Quality boundary: policy-level token parity passed, but the mixed gate/up NVFP4 path remains rejected against exact BF16. Best exact full-model decode remains `0.010559 tok/s`; historical best mixed remains `0.014484 tok/s`; no 10 tok/s result exists.
- Decision: retain adaptive as experimental/default-off and stop extending Python-only admission heuristics. The added residency is real, but synchronous per-expert sidecar decode/H2D and Python full-model orchestration remain dominant.
- Current hardware assumptions: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, 96 GB host RAM, WSL2 Ubuntu-24.04/PyTorch `2.13.0+cu130` for development; Linux native remains the deployment target.
- Latest measured bottleneck: only 27 of 1,800 full-run device lookups hit despite filling 4 GiB. Every nonresident mixed expert still requires synchronous sidecar decode/H2D, while the official 282-shard model is not connected to the C++ full-model runner.
- Known failures/blockers: physical NVMe and full-model H2D GB/token are unmeasured in the timing gate; NVFP4 final-token quality diverges from exact BF16; C++ lacks official-bundle full-model final logits; concurrent resident-table forwards need per-session ownership or serialization.
- Next concrete tasks: implement the official bundle-index/full-model C++ reader boundary, prove exact token/logit parity on a bounded layer range, then add pooled pinned asynchronous sidecar/H2D scheduling and measure physical bytes, queue stalls, cache hits, and end-to-end decode before another 78-layer gate.
- GitHub status: outside the sandbox, `gh auth status` verifies active account `rsb1813` with `repo` and `workflow` scopes and `origin=https://github.com/rsb1813/project-glm5x.git`. The earlier invalid-token/network note is superseded by this live check.
- Last known-good test state: focused cache/model/schema regression `39 passed, 6 skipped`; complete WSL Python regression `369 passed, 124 skipped` in `108.40 s`; changed-module `py_compile`, `git diff --check`, B-0004 JSON/CSV parsing, raw SHA-256 `4/4`, and generated-token parity all passed. C++ was unchanged, so prior CTest `27/27` remains the latest applicable CUDA result.

## 2026-08-16 -- Official C++ runtime-index milestone

- Current milestone: the official 282-shard GLM-5.2 K3X bundle now has a validated fixed-record C++ ingress on `codex/cpp-runtime-index`. Plan commit is `4b8a13a`, writer commit is `6284ba5`, C++ reader commit is `5511cca`, the official MTP compatibility fix is `3c21eb4`, and B-0005 evidence is committed as `bcf1cbe`.
- Completed work: atomic `glm5x-convert build-runtime-index`, deterministic/versioned `.gxi` records, header CRC32C/body SHA-256, relative-path containment, artifact root/count validation, sorted unique tensor locators, exact tensor/expert extent reads, CLI tests, C++ contract executable, corruption tests, and official layer-10 expert parity.
- Official artifact result: `build-glm5x-full-k3x/glm5x-runtime.gxi` contains 282 artifacts and 59,585 tensor locators in `1,454,984` bytes. SHA-256 is `c69cd3a4ab22f1424c201a87f014dedff9bd3b78e90d8690b0f97efb72561485`; metadata-only build wall time was `5.29 s` with `660,416 KiB` maximum RSS.
- B-0005 result: C++ opened all referenced artifact metadata and read layer 10 expert 48 in `3.31 s`, `86,784 KiB` maximum RSS, 3 reads, and `75,497,472` payload bytes. Python measured `3.36 s`, `210,184 KiB`, one grouped read, and the same bytes. All gate/up/down SHA-256 digests matched exactly.
- Correctness repair: official shard 270 contains layer-78 MTP non-expert tensors and shard 271 contains layer-78 MTP expert tensors, while the main `LAYR` directory covers `0..77`. Tensor IDs are now accepted only below `hidden_layers + mtp_layers`; expert IDs and actual main `LAYR`/`EXPT` records retain strict bounds. Focused RED-to-GREEN coverage reproduces both official cases.
- Current hardware assumptions: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, 96 GB host RAM, Solidigm P44 Pro 2 TB, WSL2 Ubuntu-24.04 for development; Linux native remains the deployment target. B-0005 did not use CUDA and did not provision cloud or paid resources.
- Latest measured bottleneck: metadata lookup is no longer the missing official-bundle boundary, but no C++ decoder consumes `.gxi` end to end yet. Exact trunk/layer construction, MLA/DSA state, natural routing, final logits, resident precision placement, and pooled asynchronous H2D still need integration. The Python full-model path remains dominated by synchronous nonresident expert sidecar/H2D traffic.
- Full-model truth: best exact measured decode remains `0.010559 tok/s`; the historical best mixed NVFP4 result remains `0.014484 tok/s` and quality-rejected. B-0005 measured no token throughput, so the requested 10 tok/s threshold remains unachieved.
- Known blockers: `.gxi` is not connected to exact decoder-layer/final-logit execution; physical NVMe and full-model H2D GB/token remain unmeasured; native/calibrated FP4 quality is unresolved; and shared C++ resident-table concurrency still needs per-session ownership or serialization.
- Next concrete tasks: use `.gxi` to construct one bounded official decoder layer without Python tensor lookup, compare layer output/final logits against the existing reference, then extend to exact multi-layer execution. Only after parity, add pooled pinned asynchronous H2D and phase-separated traffic/stall telemetry before another full 78-layer throughput gate.
- GitHub status: branch `codex/cpp-runtime-index` is published outside the sandbox and stacked draft PR [#11](https://github.com/rsb1813/project-glm5x/pull/11) targets `codex/adaptive-hot-bank`. The active account is `rsb1813`; HTTPS origin and `repo`/`workflow` scopes are live.
- Last known-good test state: C++ WSL build completed; CTest `15/15` passed; full Python regression `374 passed, 124 skipped` in `114.64 s`; focused runtime-index regression `4 passed`; official 282-shard payload parity passed; B-0005 JSON/CSV parsing and raw SHA-256 `3/3` passed; `git diff --check` passed.

## 2026-08-16 -- Index-backed learned CUDA MoE milestone

- Current milestone: the existing exact layer-10 learned-MoE CUDA boundary now consumes the official 282-shard `.gxi` directly on `codex/index-backed-cuda-moe`. Plan commit is `7f00839`, indexed tensor API commit is `82e7b47`, CUDA source integration commit is `91604c7`, and B-0006 evidence/docs commit is `cd425e3`.
- Completed work: validated metadata-plus-payload indexed reads, metadata-only tensor membership, mutually exclusive `--artifact-dir`/`--runtime-index` CLI sources, indexed router/correction-bias/routed-expert/shared-expert acquisition, additive source read telemetry, RED-to-GREEN CLI coverage, and a paired official GLM5XACT RTX 5080 gate.
- B-0006 result: both two-token rows selected the same 16 routed experts and contributions, issued 53 reads for `1,286,603,776` bytes, admitted/moved `1,283,457,024` cold weight bytes, moved zero warm weight bytes, and reported identical CPU/GPU and BF16-boundary errors. Exact indexed source integration is verified.
- Timing result: the official index row loaded payloads in `15.602102977 s` versus `16.466508213 s` for the five-artifact control, but its warm CUDA median was `2.069471 ms` versus `1.979903 ms`. This single opposite-direction pair is treated as run noise, not a throughput promotion.
- Current hardware assumptions: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, 96 GB host RAM, Solidigm P44 Pro 2 TB, WSL2 Ubuntu-24.04/CUDA 13 for development; Linux native remains the deployment target. No cloud or paid resource was used.
- Latest measured bottleneck: exact tensor lookup is no longer a blocker, but one two-token layer-10 MoE union still reads `1.2866 GB` before residency (`643,301,888` logical bytes/token for this layer only). The index feeds CUDA correctly; actual decoder-layer reuse, trunk execution, and asynchronous storage/H2D ownership are now the missing performance path.
- Full-model truth: best exact measured decode remains `0.010559 tok/s`; the historical best mixed NVFP4 result remains `0.014484 tok/s` and quality-rejected. B-0006 is a MoE sublayer benchmark and does not change the requested 10 tok/s completion status.
- Known blockers: C++ still lacks exact q-residual/MLA/DSA and final-logit integration through `.gxi`; per-invocation host payload vectors do not persist across decoder steps; physical NVMe and full-model H2D GB/token remain unmeasured; native calibrated FP4 quality remains unresolved.
- Next concrete tasks: use `read_tensor_with_metadata()` to construct the exact layer-10 trunk/attention inputs required around the proven MoE boundary, export/compare the complete decoder-layer output, then extend the same ownership to multiple layers and final logits. Add pooled pinned/asynchronous H2D only after this exact layer contract is green.
- GitHub status: parent runtime-index draft PR [#11](https://github.com/rsb1813/project-glm5x/pull/11) is published. Branch `codex/index-backed-cuda-moe` is published outside the sandbox and stacked draft PR [#12](https://github.com/rsb1813/project-glm5x/pull/12) targets `codex/cpp-runtime-index`.
- Last known-good test state: CUDA CTest `27/27` passed in `6.06 s`; focused runtime-index/CUDA-source Python `5 passed`; CPU CTest `15/15` passed; full Python `374 passed, 125 skipped` in `97.24 s`; B-0006 JSON/CSV parsing, raw SHA-256 `2/2`, route/contribution/read/error parity, runner/index SHA-256, and `git diff --check` passed.

## 2026-08-16 -- Index-backed complete decoder-layer milestone

- Current milestone: official GLM-5.2 layer 10 now executes as one bounded C++/CUDA decoder layer sourced directly from the 282-artifact `.gxi`. Plan commit is `ee1e157`, initial oracle exporter commit is `ea61a83`, and implementation/test commit is `902f551` on `codex/index-backed-cuda-moe`.
- Completed work: deterministic full-layer `GLM5XACT` oracle export, explicit CUDA/lazy-bundle metadata, 14 indexed trunk/indexer tensor loads, q-residual, official DSA Top-K telemetry, MLA causal attention, both residuals, natural Top-8 routing, exact routed/shared BF16 expert acquisition, full-layer output comparison, and B-0007 raw/summary artifacts.
- B-0007 result: two-token warm layer median `475.927299 ms` (`237.9636495 ms/token`), 67 source reads and `1,635,416,576` logical bytes, `1,632,239,616` resident weight bytes, `1,283,457,024` cold H2D bytes, and zero warm H2D bytes.
- Quality boundary: DSA Top-K and expert sets match Python. Route ordering is not bitwise-identical; maximum paired contribution delta is `0.0001886785`. Final BF16 output differs by `0.015625` absolute and `0.0042016809` relative, so this is accepted only as bounded numerical parity, not strict trace parity.
- Full-model truth: the best exact measured decode remains `0.010559 tok/s`; the historical mixed-NVFP4 result remains `0.014484 tok/s` and quality-rejected. B-0007 is one layer, so 10 tok/s remains unachieved.
- Current hardware assumptions: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, 96 GB host RAM, Solidigm P44 Pro 2 TB, WSL2 Ubuntu-24.04/CUDA 13 for development; Linux native remains the deployment target. No cloud or paid resource was used.
- Latest measured bottleneck: even with all selected weights resident and zero warm H2D, benchmark-local token-by-token trunk projections plus host normalization/softmax require `237.964 ms/token` for one layer. Production batched/fused trunk execution and multi-layer device ownership now precede more cache-policy work.
- Known blockers: route-order bitwise parity is unresolved; MLA/DSA state is not persisted across invocations; the C++ path is benchmark-local rather than the production multi-layer runner; final logits and generated tokens are not connected; process peak VRAM, physical NVMe, kernel, and I/O-stall telemetry are missing.
- Next concrete tasks: move the proven layer graph into a batched resident production decoder path, keep intermediate state on the GPU, verify a short multi-layer range and final logits against Python, then add pooled asynchronous sidecar/H2D scheduling with explicit traffic/stall measurements. Do not extrapolate B-0007 to full-model tok/s.
- GitHub status: draft PR [#12](https://github.com/rsb1813/project-glm5x/pull/12) tracks `codex/index-backed-cuda-moe` and is accessed through the verified `rsb1813` account outside the sandbox.
- Last known-good test state: implementation commit `902f551`; CUDA CTest `27/27` passed in `6.15 s`; CPU CTest `15/15` passed in `0.29 s`; focused exporter/CLI Python `5 passed`; full CPU-build Python `375 passed, 126 skipped` in `83.46 s`; B-0007 JSON/hash/DSA/expert-set checks and `git diff --check` passed.

## 2026-08-16 -- Resident BF16 dense-hit milestone

- Current milestone: commit `3b7a35a` adds an internal resident raw-BF16 dense lookup before complete host-weight conversion. The public runtime graph, arithmetic, routing, and storage format are unchanged.
- Completed work: resident key validation, hit touch/protection, pre-conversion lookup, additive host-conversion telemetry, exact repeated-output coverage, invalid-key coverage, and B-0008 two-token plus single-token official layer-10 measurements.
- B-0008 result: the two-token warm median fell from B-0007's `475.927299 ms` to `5.503443 ms`, an `86.4781x` bounded improvement. The independent one-token gate measured `2.756654 ms/layer-token`. Both optimized rows moved zero warm weight bytes.
- Quality boundary: the two-token expected-output absolute/relative error remained `0.015625` / `0.00420168088749`; the single-token row measured `0.015625` / `0.00434782588854`. DSA Top-K and routed-expert sets remain matched; route ordering remains non-bitwise-identical as recorded in B-0007.
- Full-model truth: B-0008 is a one-layer resident benchmark. Multiplying the one-token result by 78 gives an optimistic derived upper bound of `4.650751534 tok/s`, not a measured model result. The best exact measured full-model decode remains `0.010559 tok/s`, and 10 tok/s remains unachieved.
- Current hardware assumptions: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, 96 GB host RAM, Solidigm P44 Pro 2 TB, WSL2 Ubuntu-24.04/CUDA 13 for development; Linux native remains the deployment target. No cloud or paid resource was used.
- Latest measured bottleneck: approximately `87.9%` of post-fix aggregate GPU kernel time belongs to BF16 GEMV kernels. Warm host conversion, warm weight H2D, and stream synchronization are no longer the dominant bounded layer costs.
- Known blockers: the C++ decoder remains benchmark-local and one-layer; final norm/logits, generated tokens, persistent multi-layer MLA/DSA state, calibrated native FP4 quality, process peak VRAM, and end-to-end storage/H2D telemetry remain open.
- Next concrete tasks: choose the minimum native calibrated FP4 or packed/fused resident projection design that can reduce weight traffic by more than the derived `2.15x` still required for 10 tok/s, implement a bounded exact/quality-gated layer path, then extend it through a short multi-layer range and final logits before another full-model gate.
- GitHub status: implementation commit `3b7a35a` is pushed outside the sandbox to public draft PR [#12](https://github.com/rsb1813/project-glm5x/pull/12) on `codex/index-backed-cuda-moe`. The B-0008 evidence/docs commit is the next publication unit.
- Last known-good test state: CUDA CTest `27/27` passed in `5.82 s`; CPU CTest `15/15` passed in `0.31 s`; focused Python `5 passed` in `3.49 s`; the two official 100-iteration raw gates completed; JSON/CSV parsing, three raw hashes, route-set/DSA parity, summary hashes, and `git diff --check` passed.

...
Latest verified complete Python regression: `369 passed, 124 skipped` in `108.40 s` against the WSL2 CPU build. CUDA CTest remains green at the prior applicable `27/27`, and CPU CTest remains green at `15/15`; B-0004 changed only Python reference/cache code. The prior milestone text below retains historical counts for context.

## Current milestone

The exact GLM-5.2 graph, 282-shard K3X materialization, resumable conversion, Python full-bundle execution, CUDA expert sublayer boundaries, packed expert sidecars, traffic telemetry, and stable/adaptive exact device-cache policies are implemented. B-0004 filled the 4 GiB cache with repeated exact experts and increased full-run device hits `21→27`, but measured only `0.013392 tok/s`, a `0.53%` single cross-run change from the unchanged stable control. The policy remains default-off, mixed NVFP4 remains quality-rejected, and 10 tok/s is not achieved. The current implementation priority is no longer another Python cache heuristic: connect the official 282-shard bundle to the C++ full-model runtime, then add pooled pinned asynchronous sidecar/H2D scheduling with exact final-token/logit and physical traffic gates.

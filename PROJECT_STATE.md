...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, an exact q-residual/MLA/DSA/MoE layer-10 reference, a multi-layer CPU reference with final logits and greedy incremental parity, a learned-router-aware raw-BF16 CUDA MoE sublayer boundary, the portable `GLM5XACT` activation handoff, an explicit GLM SiLU path, on-demand model layer loading, shared bundle readers with indexed tensor metadata, opt-in lazy payload validation, opt-in bounded trunk-layer caching, an explicit dense-MLP path for the first three GLM-5.2 layers, a configuration-driven all-layer bundle factory, CUDA device staging/parity, resumable local full-checkpoint streaming, lazy final bundle indexing, marker-aware resume without redownloading completed shards, disjoint local shard-range workers, an opt-in parity-tested Python expert-major MoE path, an experimental opt-in sparse DSA Top-K MLA gather path, an opt-in row-scaled FP8 experiment, grouped exact expert-role reads, a bounded C++ deadline expert-load worker pool, logical storage telemetry, exact FP32 LM-head reuse with BF16 source release, and an explicit full-bundle benchmark gate are implemented. Large role payloads use a read-only memory view for the tensor decode path while tiny fixtures retain a writable fallback. The latest locally verified implementation is `9aea39d`; public Linux correctness `31857947649` and CodeQL `31857947641` both passed, C++ CTest is green at `15/15`, and the full Python suite is green at `327 passed, 124 skipped`. At the 2026-08-15 11:03 KST snapshot, 238 of 282 real shards had finalized and C: had approximately 411 GB free. The recent worker rate remains around 45 shards/hour, leaving 44 shards and roughly 1 hour; allowing for rate variance, lazy indexing, and the first cold/cached CUDA gate, the first full-bundle reference window is approximately 12:00--14:00 KST. This is an estimate, not a completion promise. Full real-checkpoint logits, MTP, CUDA final logits, and end-to-end tok/s remain unmeasured.

## Current live snapshot -- 2026-08-15 11:03 KST
...
- Materialization: 238 `.k3x` artifacts and 238/282 atomic source-deleted markers are present; no bounded `.partial` files are active at the probe. Completed sources are not redownloaded.
- Live conversion refresh: three WSL2 workers remain active on disjoint ranges. The observed sustained range is approximately 45 shards/hour, with transient variation from shard size and NTFS I/O. There are 44 source-deletion markers left.
- Storage: C: has approximately 411 GB free at this observation. Each raw-BF16 `.k3x` shard is about 5.366 GB, so the remaining shards need roughly 236 GB and the final raw bundle is expected to be about 1.5 TB. No unrelated large writes should be started.
- Conversion estimate: at the observed range, shard conversion reaches 282 in roughly 1 hour from this snapshot. Lazy bundle indexing and the first cold/cached CUDA reference gates add an unmeasured post-conversion phase; the first complete-run window is therefore approximately 12:00--14:00 KST.
...
- GitHub: Linux correctness `31857947649` and CodeQL `31857947641` both passed for public head `9aea39d`; earlier public heads `1a236f2`, `f4fbcc1`, and `2a9778d` also passed. The recurring red Linux notifications point to stale run `31795400168` on `b94c8b8`, where 50 historical evidence files were absent, not an active failure on `main`.
...
- Full GLM-5.2 local materialization is in progress. 238 of 282 shards are finalized as verified `.k3x` artifacts and their source-deleted markers are present; full checkpoint correctness and local TPS are not measured yet.

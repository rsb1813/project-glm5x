...
- Materialization: 235 `.k3x` artifacts and 235/282 atomic source-deleted markers are present; no bounded `.partial` files are active at the probe. Completed sources are not redownloaded.
- Live conversion refresh: three WSL2 workers remain active on disjoint ranges. The observed sustained range is approximately 45 shards/hour, with transient variation from shard size and NTFS I/O. There are 47 source-deletion markers left.
- Storage: C: has approximately 432 GB free at this observation. Each raw-BF16 `.k3x` shard is about 5.366 GB, so the remaining shards need roughly 252 GB and the final raw bundle is expected to be about 1.5 TB. No unrelated large writes should be started.
...
- GitHub: Linux correctness `31857658252` and CodeQL `31857658293` both passed for public head `1a236f2`; earlier public heads `f4fbcc1` and `2a9778d` also passed. The recurring red Linux notifications point to stale run `31795400168` on `b94c8b8`, where 50 historical evidence files were absent, not an active failure on `main`.
...
- Full GLM-5.2 local materialization is in progress. 235 of 282 shards are finalized as verified `.k3x` artifacts and their source-deleted markers are present; full checkpoint correctness and local TPS are not measured yet.

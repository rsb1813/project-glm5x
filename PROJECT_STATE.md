...
- Materialization: 232 `.k3x` artifacts and 232/282 atomic source-deleted markers are present; no bounded `.partial` files are active at the probe. Completed sources are not redownloaded.
- Live conversion refresh: three WSL2 workers remain active on disjoint ranges. The observed sustained range is approximately 35--45 shards/hour, with transient variation from shard size and NTFS I/O. There are 50 source-deletion markers left.
- Storage: C: has approximately 443 GB free at this observation. The projected final output still fits only with a shrinking margin, so free space must be rechecked before final assembly and no unrelated large writes should be started.
- Conversion estimate: at the observed range, shard conversion reaches 282 in roughly 1.1--1.5 hours from this snapshot. Lazy bundle indexing and the first cold/cached CUDA reference gates add an unmeasured post-conversion phase; the first complete-run window is therefore approximately 12:10--13:35 KST.
...
- GitHub: Linux correctness `31857433809` and CodeQL `31857433741` both passed for public head `f4fbcc1`; the traffic-documentation head `2a9778d` also passed correctness `31856761733` and CodeQL `31856761734`. The recurring red Linux notifications point to stale runs such as `31795400168` on `b94c8b8`, not an active failure on `main`.

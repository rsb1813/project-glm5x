...
- Materialization: 115/282 `.k3x` artifacts and 115/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: the latest monitor samples vary around 34--40 shards/hour, leaving 167 artifacts. The conservative conversion/indexing window is about 4.5--5.5 hours from this snapshot because network, NTFS, and retry behavior are not stationary.
...
- GitHub: `9cf5822` Linux correctness `31848491151` and CodeQL `31848491142` both completed successfully. The recurring red Linux notifications point to stale run `31795400168` on `b94c8b8`, not an active failure on `main`.

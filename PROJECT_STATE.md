...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, the exact q-residual/MLA/DSA/MoE reference, configuration-driven all-layer loading, resumable local full-checkpoint streaming, lazy final bundle indexing, disjoint shard-range workers, exact cache/prefetch experiments, CUDA expert-major sublayer boundaries, logical storage telemetry, grouped decoder-layer reads, exact FP32 LM-head reuse, reduced-routing/proxy controls, FP8/INT4 sidecars, and the explicit full-bundle benchmark gate are implemented. The latest local regression is `334 passed, 124 skipped`; the C++ suite remains green at `15/15`. All 282 real shards now have verified `.k3x` artifacts and source-deletion markers, and lazy assembly reports 19,456 complete experts. The first exact full-model CUDA gate is measured: cold prefill `0.003258 tok/s`, cold decode `0.003304 tok/s`, TTFT `609.621 s`, and logical storage reads `79,763,152,896` bytes per token. The cached two-token gate remains exact but records `0.003271 tok/s`, `0.0` expert-cache hit rate, and `159,526,305,792` decode bytes because the current 8 GiB host/4 GiB device capacity is smaller than the working set and the trunk is still reloaded. These are baseline measurements, not a target claim. Full CUDA final-logit quality, MTP, paged KV, asynchronous residency, calibrated FP4, and optimized end-to-end execution remain open.
...
- Current bottleneck: storage read/reload, not CUDA arithmetic. The exact reference is operational for a correctness baseline, but it is not yet an interactive local engine. The next implementation target is resident trunk reuse and layer-aware pinned/asynchronous staging, followed by a fresh measured gate.
...
- Verification: focused proxy/model coverage passed `24 passed, 6 skipped`; the complete WSL Python suite passed `334 passed, 124 skipped` in `76.00 s`. These are current local results; no full-model speed claim changed.
- GitHub: public head `2c1db69` contains the reduced-routing/proxy experiment. Its local verification is `333 passed, 124 skipped`; the latest published CI result recorded in this document remains the earlier successful correctness/CodeQL pair. The recurring red `correctness / Linux (push)` notification is stale run `31795400168` on `b94c8b8`, where 50 historical evidence files were absent; it is not an active failure on `main`.

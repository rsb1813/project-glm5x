...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, an exact q-residual/MLA/DSA/MoE layer-10 reference, a multi-layer CPU reference with final logits and greedy incremental parity, a learned-router-aware raw-BF16 CUDA MoE sublayer boundary, the portable `GLM5XACT` activation handoff, an explicit GLM SiLU path, on-demand model layer loading, shared bundle readers with indexed tensor metadata, opt-in lazy payload validation, opt-in bounded trunk-layer caching, an explicit dense-MLP path for the first three GLM-5.2 layers, a configuration-driven all-layer bundle factory, CUDA device staging/parity, resumable local full-checkpoint streaming, lazy final bundle indexing, marker-aware resume without redownloading completed shards, disjoint local shard-range workers, an opt-in parity-tested Python expert-major MoE path, an experimental opt-in sparse DSA Top-K MLA gather path, an opt-in row-scaled FP8 experiment, grouped exact expert-role reads, a bounded C++ deadline expert-load worker pool, logical storage telemetry, exact FP32 LM-head reuse, and an explicit full-bundle benchmark gate are implemented. Large role payloads use a read-only memory view for the tensor decode path while tiny fixtures retain a writable fallback. The latest locally verified implementation is `70d7d39` with traffic documentation at `2a9778d`; public Linux correctness and CodeQL both passed, C++ CTest is green at `15/15`, and the full Python suite is green at `327 passed, 124 skipped`. At the 2026-08-15 10:36 KST snapshot, 223 of 282 real shards had finalized and C: had approximately 477 GB free. The recent worker rate remains around 30--40 shards/hour, leaving 59 shards and roughly 1.5--2.0 hours; allowing for rate variance, lazy indexing, and the first cold/cached CUDA gate, the first full-bundle reference window is approximately 12:20--13:50 KST. This is an estimate, not a completion promise. Full real-checkpoint logits, MTP, CUDA final logits, and end-to-end tok/s remain unmeasured.

## Current live snapshot -- 2026-08-15 10:36 KST
...
- Materialization: 224 `.k3x` artifacts and 223/282 atomic source-deleted markers are present; no bounded `.partial` files are active at the probe. Completed sources are not redownloaded.
- Live conversion refresh: three WSL2 workers remain active on disjoint ranges. The observed sustained range is approximately 30--40 shards/hour, with transient variation from shard size and NTFS I/O. There are 59 source-deletion markers left.
- Storage: C: has approximately 477 GB free at this observation. The projected final output still fits only with a shrinking margin, so free space must be rechecked before final assembly and no unrelated large writes should be started.
- Conversion estimate: at the observed range, shard conversion reaches 282 in roughly 1.5--2.0 hours from this snapshot. Lazy bundle indexing and the first cold/cached CUDA reference gates add an unmeasured post-conversion phase; the first complete-run window is therefore approximately 12:20--13:50 KST.
...
- GitHub: Linux correctness `31856761733` and CodeQL `31856761734` both passed for public head `2a9778d`; the LM-head implementation head `70d7d39` also passed correctness `31856623891` and CodeQL `31856623886`. The recurring red Linux notifications point to stale runs such as `31795400168` on `b94c8b8`, not an active failure on `main`.

...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, an exact q-residual/MLA/DSA/MoE layer-10 reference, a multi-layer CPU reference with final logits and greedy incremental parity, a learned-router-aware raw-BF16 CUDA MoE sublayer boundary, the portable `GLM5XACT` activation handoff, an explicit GLM SiLU path, on-demand model layer loading, shared bundle readers, opt-in lazy payload validation, opt-in bounded trunk-layer caching, an explicit dense-MLP path for the first three GLM-5.2 layers, a configuration-driven all-layer bundle factory, CUDA device staging/parity, resumable local full-checkpoint streaming, lazy final bundle indexing, marker-aware resume without redownloading completed shards, disjoint local shard-range workers, an opt-in parity-tested Python expert-major MoE path, and an explicit full-bundle benchmark gate are implemented. The latest locally verified implementation is `7c79976`; the focused MLA/layer/model suite is green. Public Linux correctness and CodeQL remain green on the preceding docs head. At the 2026-08-15 07:20 KST snapshot, 107 of 282 real shards had finalized. The recent worker rate varies around 34--40 shards/hour, leaving 175 shards and roughly 4.4--5.2 hours; allowing for rate variance and final indexing, the first full-bundle reference gate is expected around 12:00--13:45 KST. This is an estimate, not a completion promise. Full real-checkpoint logits, MTP, CUDA final logits, and end-to-end tok/s remain unmeasured.

## Current live snapshot -- 2026-08-15 07:20 KST
...
- Materialization: 107/282 `.k3x` artifacts and 107/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: the latest monitor samples vary around 34--40 shards/hour, leaving 175 artifacts. The conservative conversion/indexing window is about 5--6 hours from this snapshot because network, NTFS, and retry behavior are not stationary.
- Storage: C: has approximately 1.148 TB decimal free; the current K3X output is approximately 0.575 TB and the active source remainder is bounded. The projected final output still fits with an estimated roughly 0.16--0.18 TB margin, but free space must be rechecked before final assembly.
- Conversion estimate: at the observed range, shard conversion reaches 282 around 11:45--12:35 KST. Lazy bundle indexing and the first cold/cached CUDA reference gates add an unmeasured but expected short post-conversion phase; the first complete-run window is therefore approximately 12:00--13:45 KST.
...
- GitHub: `8f541fa` Linux correctness `31845394704` and CodeQL `31845394702` both completed successfully. The recurring red Linux notifications point to stale run `31795400168` on `b94c8b8`, not an active failure on `main`.

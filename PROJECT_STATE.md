...
- Materialization: 72/282 `.k3x` artifacts and 72/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: 62 artifacts were produced after the 04:39:53 three-worker launch, ending at 06:24:02. The observed aggregate is approximately 35.8 shards/hour, leaving 210 shards and roughly 5.9 hours at that rate; plan 6--7 hours for variance and retries.
...
- Conversion estimate: 62 artifacts completed after the parallel launch at 04:39:53, ending at 06:24:02. The 35.8-shards/hour sample implies about 5.9 hours for the remaining 210 artifacts; a conservative planning window is 6--7 hours because network, NTFS, and retry behavior are not stationary.
...
- GitHub: `0e6972d` Linux correctness `31842116346` and CodeQL `31842116338` both completed successfully. The recurring red Linux notification is historical, not an active failure on `main`.

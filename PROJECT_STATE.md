...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, an exact q-residual/MLA/DSA/MoE layer-10 reference, a multi-layer CPU reference with final logits and greedy incremental parity, a learned-router-aware raw-BF16 CUDA MoE sublayer boundary, the portable `GLM5XACT` activation handoff, an explicit GLM SiLU path, on-demand model layer loading, shared bundle readers with indexed tensor metadata, opt-in lazy payload validation, opt-in bounded trunk-layer caching, an explicit dense-MLP path for the first three GLM-5.2 layers, a configuration-driven all-layer bundle factory, CUDA device staging/parity, resumable local full-checkpoint streaming, lazy final bundle indexing, marker-aware resume without redownloading completed shards, disjoint local shard-range workers, an opt-in parity-tested Python expert-major MoE path, an experimental opt-in sparse DSA Top-K MLA gather path, an opt-in row-scaled FP8 experiment, grouped exact expert-role reads, and an explicit full-bundle benchmark gate are implemented. Large role payloads use a read-only memory view for the tensor decode path while tiny fixtures retain a writable fallback. The latest locally verified implementation is `f760934`; the focused GLM expert-bundle/reader/layer/model suite is green (`59 passed` before the grouped-read-only change). At the 2026-08-15 08:38 KST snapshot, 153 of 282 real shards had finalized. The recent worker rate remains around 34--40 shards/hour, leaving 129 shards and roughly 3.2--3.8 hours; allowing for rate variance, lazy indexing, and the first cold/cached CUDA gate, the first full-bundle reference window is approximately 12:30--13:30 KST. This is an estimate, not a completion promise. Full real-checkpoint logits, MTP, CUDA final logits, and end-to-end tok/s remain unmeasured.

## Current live snapshot -- 2026-08-15 08:38 KST
...
- Materialization: 153/282 `.k3x` artifacts and 153/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: the latest sample advanced from 142 to 153 finalized artifacts, consistent with the wider 34--40 shards/hour range. There are 129 artifacts left; the conservative conversion/indexing window is about 3.2--3.8 hours from this snapshot because network, NTFS, and retry behavior are not stationary.
- Storage: C: has approximately 1.008 TB decimal free; the current K3X output is approximately 0.710 TB and the active source remainder is bounded. The projected final output still fits, but free space must be rechecked before final assembly.
- Conversion estimate: at the observed range, shard conversion reaches 282 around 11:50--12:30 KST. Lazy bundle indexing and the first cold/cached CUDA reference gates add an unmeasured but expected short post-conversion phase; the first complete-run window is therefore approximately 12:30--13:30 KST.
...
- GitHub: Linux correctness `31851022001` and CodeQL `31851021942` passed for `f760934`. The recurring red Linux notifications point to stale runs such as `31795400168` on `b94c8b8`, not an active failure on `main`.
...
- The latest pushed head is `f760934`; Linux correctness `31851022001` and CodeQL `31851021942` both completed successfully. The old red notification run `31795400168` failed because historical `results/b0006..b0024` evidence files were absent on commit `b94c8b8`; it is not a failure of the current `main` head.

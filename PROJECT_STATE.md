...
GLM-5.2 shape/manifest boundary, exact cross-shard raw-BF16 loading, an exact q-residual/MLA/DSA/MoE layer-10 reference, a multi-layer CPU reference with final logits and greedy incremental parity, a learned-router-aware raw-BF16 CUDA MoE sublayer boundary, the portable `GLM5XACT` activation handoff, an explicit GLM SiLU path, on-demand model layer loading, shared bundle readers with indexed tensor metadata, opt-in lazy payload validation, opt-in bounded trunk-layer caching, an explicit dense-MLP path for the first three GLM-5.2 layers, a configuration-driven all-layer bundle factory, CUDA device staging/parity, resumable local full-checkpoint streaming, lazy final bundle indexing, marker-aware resume without redownloading completed shards, disjoint local shard-range workers, an opt-in parity-tested Python expert-major MoE path, an experimental opt-in sparse DSA Top-K MLA gather path, and an explicit full-bundle benchmark gate are implemented. Large role payloads use a read-only memory view for the tensor decode path while tiny fixtures retain a writable fallback. The latest locally verified implementation is `214590b`; the focused expert-bundle/MLA/layer/model/MoE suite is green (`16/16` with no warnings). At the 2026-08-15 08:16 KST snapshot, 139 of 282 real shards had finalized. The recent worker rate varies around 34--40 shards/hour, leaving 143 shards and roughly 3.6--4.2 hours; allowing for rate variance and final indexing, the first full-bundle reference gate is expected around 12:30--13:30 KST. This is an estimate, not a completion promise. Full real-checkpoint logits, MTP, CUDA final logits, and end-to-end tok/s remain unmeasured.

## Current live snapshot -- 2026-08-15 08:16 KST
...
- Materialization: 139/282 `.k3x` artifacts and 139/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: the latest 30-minute sample is 17 artifacts, approximately 34 shards/hour, consistent with the wider 34--40 shards/hour range. There are 143 artifacts left; the conservative conversion/indexing window is about 3.6--4.2 hours from this snapshot because network, NTFS, and retry behavior are not stationary.
...
- GitHub: Linux correctness `31849485242` and CodeQL `31849485259` passed for `1ff787b`. The recurring red Linux notifications point to stale run `31795400168` on `b94c8b8`, not an active failure on `main`.

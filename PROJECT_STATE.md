...
- Materialization: 132/282 `.k3x` artifacts and 132/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: the latest monitor samples vary around 34--40 shards/hour, leaving 150 artifacts. The conservative conversion/indexing window is about 4.0--4.5 hours from this snapshot because network, NTFS, and retry behavior are not stationary.
...
- Conversion estimate: at the observed range, shard conversion reaches 282 around 12:15--12:40 KST. Lazy bundle indexing and the first cold/cached CUDA reference gates add an unmeasured but expected short post-conversion phase; the first complete-run window is therefore approximately 12:30--13:30 KST.
...
- GitHub: `214590b` has just been pushed; its Linux correctness and CodeQL runs are pending. The recurring red Linux notifications point to stale run `31795400168` on `b94c8b8`, not an active failure on `main`.

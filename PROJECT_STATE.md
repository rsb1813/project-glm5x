...
- Materialization: 160/282 `.k3x` artifacts and 160/282 atomic source-deleted markers are present. Active workers keep only bounded in-progress source files and interruption-safe `.part` state; completed sources are not redownloaded.
- Live conversion refresh: the latest sample advanced from 153 to 160 finalized artifacts, consistent with the wider 33--40 shards/hour range. There are 122 artifacts left; the conservative conversion/indexing window is about 3.1--3.7 hours from this snapshot because network, NTFS, and retry behavior are not stationary.
...
- GitHub: Linux correctness `31851436292` and CodeQL `31851436264` passed for `d539551` (Linux 2m54s; CodeQL Python 2m11s and C++ 3m41s). The recurring red Linux notifications point to stale runs such as `31795400168` on `b94c8b8`, not an active failure on `main`.

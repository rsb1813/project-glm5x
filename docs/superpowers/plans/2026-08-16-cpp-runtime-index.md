...
- [ ] Write a fixture test that independently decodes literal header fields and verifies deterministic bytes.
- [ ] Run it and confirm RED on the missing module/API.
- [ ] Implement atomic writer, CRC32C, SHA-256, duplicate checks, and CLI `build-runtime-index`.
- [ ] Run focused writer/CLI tests and confirm GREEN.
- [ ] Commit the writer and tests.
...
- [ ] Write the C++ executable contract test and add its build target.
- [ ] Build first and confirm RED on the missing public interface.
- [ ] Implement strict fixed-record parsing, range/overflow checks, reserved-zero checks, path containment, and sorted unique tensor lookup.
- [ ] Validate each opened artifact's root SHA, tensor count, record index, tensor ID, and data CRC metadata before accepting the index.
- [ ] Reuse the existing exact BF16 expert shape/dtype/CRC contract for three-role reads.
- [ ] Run the C++ fixture through Pytest and confirm GREEN.
- [ ] Commit the C++ reader and integration tests.

### Task 3: Official 282-shard bounded gate

**Files:**
- Modify: `tests/python/test_glm5x_runtime_index.py`
- Create: `tools/benchmark_glm5x_runtime_index.py`
...
- [ ] Build the official `.gxi` without payload/root rescans and record its SHA-256.
- [ ] Open all 282 metadata shards through C++ and load one real layer-10 expert.
- [ ] Compare all three exact role payload digests/lengths with Python.
- [ ] Record startup/load telemetry without extrapolating token throughput.
- [ ] Run focused C++/Python regression and applicable full suites.
...
- [ ] Record implemented/proposed boundaries and B-0005 measurements.
- [ ] Verify raw hashes, JSON/CSV parity, compile/tests, and `git diff --check`.

...
def test_multi_shard_conversion_deletes_verified_sources_and_resumes_from_markers(tmp_path) -> None:
    source_dir = tmp_path / "source"
    output_dir = tmp_path / "output"
    source_dir.mkdir()
    names = {
    ...
    first = convert_glm5x_shards(
        source_dir, output_dir, manifest, chunk_bytes=17, delete_source=True
    )
    second = convert_glm5x_shards(
        source_dir, output_dir, manifest, chunk_bytes=17, delete_source=True
    )

    assert first.completed is True
    assert first.deleted_shards == manifest.shard_names
    assert all(not path.exists() for path in sources)
    assert second.completed is True
    assert second.skipped_shards == manifest.shard_names
    assert second.deleted_shards == manifest.shard_names

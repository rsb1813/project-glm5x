...
## 2026-08-16 -- B-0002 stable hot-bank real-sidecar residency gate

- Date/commit: 2026-08-16; measured on the uncommitted `codex/stable-hot-bank` working tree based on `0437bc0`.
- Hardware: NVIDIA GeForce RTX 5080 16 GB, WSL2 Ubuntu-24.04, CUDA-enabled PyTorch `2.13.0+cu130`.
- Model/checkpoint: official GLM-5.2 K3X bundle `build-glm5x-full-k3x/glm5x-experts-full.json`, SHA-256 `c75b96f8b7be54603fcd7755f7d821814116907cf6e0bc3f5b65f55a4dc741c8`; exact digest-matched mixed `.pgu` sidecars with NVFP4 gate/up and BF16 down.
- Mode/context: structured repeated-residency trace over layers `3..18`, eight experts per layer, 128 accesses per pass, 768 MiB decoded device cache, 8 GiB validated host cache, one cold pass plus three warm passes. This trace is not natural-router full-model decode.
- LRU: cold `29.795967729 s`; warm median `3.656221967 s`; `0/128` warm hits, 128 evictions per warm pass, `5,033,165,824` H2D bytes, `786,432,160` resident bytes, and `825,775,104` peak CUDA allocated bytes.
- Stable hot bank with one retained entry per layer: cold `30.084442793 s`; warm median `3.289269276 s`; `16/128` warm hits, zero evictions, 112 bypasses, `4,404,020,096` H2D bytes, `629,145,728` resident bytes, and `668,484,608` peak CUDA allocated bytes.
- Comparison: warm median wall time decreased `10.036%`, H2D bytes decreased `12.5%`, and peak CUDA allocation decreased `19.048%`. Median summed CUDA-event H2D time changed only `572,164,194` to `562,860,753 ns`, so the wall improvement must not be attributed solely to transfer bandwidth.
- Correctness/quality: sidecars were accepted only when their source digest matched the bundle. The policy returns the same exact packed/BF16 payload objects and never changes routing, but this benchmark did not compute logits or generated tokens; no quality or full-model TPS claim is made.
- Metrics not applicable or not measured: decode/prefill tok/s, TTFT, physical NVMe GB/token, system-RAM peak, average Top-K, speculative acceptance, and coding benchmark quality.
- Artifacts: `results/b0002-stable-hot-bank-rtx5080/lru-repeat.json`, `stable-hot-bank-repeat.json`, `summary.json`, and `summary.csv`. Raw JSON SHA-256 values are `0dca3c1f3fcb38789ca0207e260ec7fe958cfcd1ec2cebf119b7693769e2616d` and `248c1a8886c9a324a4bd399c3f775c889ce0a6ff2f6585a033b7ba6259c44261`.
- Decision: retain the policy as experimental and default-off. The next gate must use natural full-model routing and pooled asynchronous H2D before another full 78-layer throughput claim.
- Verification: B-0002 raw SHA-256 and warm-median summary parity passed; focused cache/model/CLI regression passed `35 passed, 6 skipped`; complete CPU-build Python regression passed `365 passed, 124 skipped` in `107.34 s`; changed-module `py_compile` and `git diff --check` passed.

## 2026-08-16 -- B-0003 full-model stable hot-bank A/B

- Date/commit: 2026-08-16; both comparable rows were measured on plan commit `0be39c8` from branch `codex/full-stable-hot-bank-gate`.
- Hardware: Ryzen 7 9800X3D host, NVIDIA GeForce RTX 5080 16 GB, 96 GB physical host RAM with `75,880,824,832` bytes visible to WSL2, Ubuntu-24.04, PyTorch `2.13.0+cu130`.
- Model/checkpoint: official 78-layer GLM-5.2 full K3X bundle, one-token prompt `[0]`, two decode tokens, natural Top-8, mixed `.pgu` gate/up NVFP4 plus BF16 down, BF16 trunk, no proxy, no sparse attention, 40 GiB trunk cache, 4 GiB decoded expert device cache, 16 expert readers, warm sidecars.
- Layer-balanced: prefill `304.412908913 s` (`0.003285011807 tok/s`), TTFT `382.766628255 s`, decode `151.924624634 s` (`0.013164422850 tok/s`), device hits/misses `18/1782`, hit rate `1.00%`, 1,673 evictions, `4,286,055,272` resident cache bytes, and `12,371,738,112` peak allocated VRAM bytes.
- Stable hot bank: prefill `301.920423536 s` (`0.003312131019 tok/s`), TTFT `379.710722871 s`, decode `150.135832250 s` (`0.013321270279 tok/s`), device hits/misses `21/1779`, hit rate `1.167%`, 54 evictions/promotions, 1,650 bypasses, `2,949,120,600` resident cache bytes, and `11,034,244,608` peak allocated VRAM bytes.
- Same-HEAD comparison: stable was `1.191%` higher in decode tok/s and `0.826%` higher in prefill tok/s, with three additional hits. It reduced resident cache bytes `31.193%` and peak allocated VRAM `10.811%`, but left `1,345,846,696` of configured device-cache capacity unused. The throughput difference is one paired sample and may include run noise.
- Correctness/quality: both policies generated `[154820,474]`. This proves policy token parity within the mixed path, but the mixed NVFP4 path remains quality-rejected because it diverges from exact BF16.
- Traffic: logical K3X decode reads were zero and packed-sidecar hits/misses/writes were `1779/0/0` for stable and `1782/0/0` for layer-balanced. Physical NVMe and H2D bytes were not measured because event telemetry was disabled for timing comparability.
- Repair run: the first stable invocation rejected and rebuilt four stale sidecars; it is preserved but excluded from A/B. Its raw SHA-256 is `97120ebff976608eaac0641fa66acd89ad2cb2894b03d7fffcfa4a5f0e5e20b1`.
- Artifacts: `results/b0003-full-stable-hot-bank-rtx5080/{stable-repair.log,stable-warm.log,layer-balanced-warm.log,summary.json,summary.csv}`. Comparable raw hashes are `8f06f3548ccde32cd39e5b20017cb4b6f82a3fa873773e064fb805aa8af95840` and `0ac5a0efa0de3e3028f482ccaaff3788e16d74da54d9b1cc095223579bace88c`.
- Decision: stable remains experimental/default-off. The next bounded implementation will use the `1.346 GB` idle budget for a repeated-expert adaptive tier; 10 tok/s is not achieved.

## 2026-08-16 -- B-0004 adaptive hot-bank bounded and full gates

- Date/implementation: 2026-08-16; adaptive implementation commit `3e0be9f` on `codex/adaptive-hot-bank`.
- Hardware/model: Ryzen 7 9800X3D, NVIDIA GeForce RTX 5080 16 GB, WSL2 Ubuntu-24.04, PyTorch `2.13.0+cu130`; official GLM-5.2 K3X bundle and digest-matched mixed `.pgu` gate/up NVFP4 plus BF16 down sidecars.
- Bounded mode: deterministic 16-layer×8-expert repeated trace, 768 MiB device cache, 8 GiB validated host cache, one cold plus three warm passes. Stable measured `16/128` steady hits, `4,404,020,096` H2D bytes, and `3.268563048 s` warm median. Adaptive measured `20/128`, `4,246,733,664` bytes, and `3.431929617 s`.
- Bounded comparison: adaptive added four exact hits and reduced steady H2D `3.571%`, but warm median wall time worsened `4.998%`. This is payload/cache evidence only; no logits or tok/s were measured.
- Full mode: 78 layers, prompt `[0]`, two decode tokens, natural Top-8, BF16 trunk, 40 GiB trunk cache, 4 GiB decoded device cache, 16 readers, and warm sidecars. Adaptive measured prefill `302.010376570 s` (`0.003311144509 tok/s`), TTFT `379.660062301 s`, and decode `149.340913192 s` (`0.013392177383 tok/s`).
- Full cache/resources: hits/misses `27/1773`, hit rate `1.5%`, 34 extra promotions, zero evictions, 1,664 bypasses, 109 entries, `4,286,055,272` resident cache bytes, and `11,642,743,296` peak allocated VRAM bytes.
- Control comparison: the unchanged ancestor stable control from B-0003 measured `0.013321270279 tok/s`, 21 hits, 75 entries, and `11,034,244,608` peak VRAM bytes. Adaptive is directionally `0.532%` faster with six extra hits and `608,498,688` more peak VRAM bytes, but this is a single cross-run comparison and is within plausible run noise rather than a promotion result.
- Correctness/quality: both cache policies generated `[154820,474]`, preserving policy-level token parity. The mixed NVFP4 path remains quality-rejected against exact BF16; best exact full-model decode remains `0.010559 tok/s`, historical best mixed remains `0.014484 tok/s`, and 10 tok/s is not achieved.
- Traffic gaps: physical NVMe GB/token, full-model H2D GB/token, peak system RAM, coding quality, and speculative acceptance were not measured in the timing gate.
...
## 2026-08-16 -- B-0006 index-backed learned CUDA MoE gate

- Date/implementation: 2026-08-16; runtime-index metadata API commit `82e7b47` and CUDA source integration commit `91604c7` on `codex/index-backed-cuda-moe`.
- Hardware/environment: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, WSL2 Ubuntu-24.04, CUDA 13. No cloud or paid resource was used.
- Model/boundary: official GLM-5.2 layer 10; two-token `GLM5XACT` MoE input/output; natural sigmoid Top-8 with routed scale 2.5; 16 unique routed experts/16 assignments plus one shared expert; raw BF16 weights; FP32 output; device accumulation and fused shared contribution; 2 GiB resident budget; five warmups and 20 measured iterations.
- Control source: five bounded `.k3x` artifacts. It issued 53 source reads for `1,286,603,776` bytes, loaded payloads in `16,466,508,213 ns`, measured `409,237,743 ns` cold and `1,979,903 ns` warm median, admitted/moved `1,283,457,024` cold weight bytes, and moved zero warm weight bytes.
- Runtime-index source: official `.gxi` with 282 referenced artifacts. It issued the same 53 reads and `1,286,603,776` bytes, loaded payloads in `15,602,102,977 ns`, measured `427,015,884 ns` cold and `2,069,471 ns` warm median, admitted/moved the same `1,283,457,024` cold bytes, and moved zero warm bytes.
- Correctness: route IDs and contributions were identical. Both rows reported GPU/CPU maximum absolute/relative error `0.0000180910574272` / `0.00045266628149` and BF16-boundary maximum absolute/relative error `0.00006103515625` / `0.00152439018711`.
- Comparison: the index row's host payload load was `5.249%` lower, while cold and warm CUDA timings were `4.344%` and `4.524%` higher. This is one paired run and the execution path after weight acquisition is identical, so these timing differences are treated as run noise rather than a speed result.
- Bottleneck: one two-token MoE sublayer still reads `1.2866 GB` from host storage/cache before residency. This is `643,301,888` logical bytes per token for this layer only; it is not a full-model GB/token measurement. Exact multi-layer residency and overlap remain mandatory.
- Not measured: decode/prefill tok/s, TTFT, process peak VRAM, system RAM peak, physical NVMe GB/token, full-model H2D GB/token, coding quality, and speculative acceptance. The best exact full-model result remains `0.010559 tok/s`.
- Artifacts: `results/b0006-index-backed-cuda-moe-rtx5080/{artifact-directory.json,runtime-index.json,summary.json,summary.csv}`. Raw SHA-256 values are `2a68cae8668d768da85c58b95644718ca62617bb7f07ac63fb398601706473a0` and `0ca233f60564f9c43cea84573c8ed604f9318ef8c33e5f5c3a9bcffc99ec7b30`.
- Verification: CUDA CTest `27/27`, focused Python `5 passed`, CPU CTest `15/15`, full Python `374 passed, 125 skipped`, B-0006 JSON/CSV/hash/parity checks, and `git diff --check` passed.
- Decision: accept `.gxi` as an exact source for the existing CUDA MoE boundary. Next implement exact decoder-layer/final-logit integration; do not infer tok/s from B-0006.

## 2026-08-16 -- B-0007 index-backed complete decoder-layer gate

- Date/commit: 2026-08-16; implementation commit `902f551` on `codex/index-backed-cuda-moe`.
- Hardware/environment: AMD Ryzen 7 9800X3D, NVIDIA RTX 5080 16 GB, 96 GB host RAM, WSL2 Ubuntu-24.04, CUDA 13. No cloud or paid resource was used.
- Model/mode/context: official GLM-5.2 layer 10 from full-layer input through input RMSNorm, q-residual, full DSA indexer, MLA causal attention, both residuals, natural sigmoid Top-8 routed MoE plus shared expert, and full-layer output. Context length was two tokens with empty incoming MLA/DSA state; precision was BF16-rounded with FP32 CUDA output, 2 GiB resident budget, 64 MiB cuBLAS workspace, three warmups, and seven measured iterations.
- Correctness: DSA Top-K was exactly `[[0,1],[1,0]]` on both paths. Both token rows selected the same expert sets as Python; route ordering differed for near-tied values, and the maximum contribution delta paired by expert was `0.0001886785`. CUDA versus the exported BF16 layer output measured maximum absolute/relative error `0.015625` / `0.00420168088749`.
- Timing: host payload load was `13.562852242 s`; one initial decoder preparation was `3.868464681 s`; cold full-layer execution was `1.116432256 s`; the warm two-token median was `475.927299 ms`, or `237.9636495 ms/token`. The derived bounded layer rate is `4.202322691 layer-tokens/s`; it is not full-model decode tok/s.
- Traffic/residency: 67 logical source reads transferred `1,635,416,576` bytes (`817,708,288` bytes/token). Cold weight H2D was `1,283,457,024` bytes (`641,728,512` bytes/token); warm weight H2D was zero. Resident weight bytes were `1,632,239,616`, including `348,812,800` decoder trunk bytes, `1,207,959,552` routed-expert host payload bytes, `75,497,472` shared-expert bytes, and `3,146,752` router bytes.
- Routing/cache/speculation: average Top-K was 8, the two-token union contained 16 experts, and no speculative decoding was enabled. Process peak VRAM, system-RAM peak, physical NVMe GB/token, kernel time, and I/O stall time were not separately measured.
- Full-model metrics: decode tok/s, prefill tok/s, TTFT, coding quality, and speculative acceptance were not measured. The best exact full-model decode therefore remains `0.010559 tok/s`; 10 tok/s is not achieved.
- Artifacts: `results/b0007-index-backed-decoder-layer-rtx5080/{full-layer.json,summary.json,summary.csv}`. Raw JSON SHA-256 is `6d4052f9e25775888dd8030ddb1d1b8f2d938c0fd96bbc1ec5c474a017ee5763`; input/output/metadata oracle SHA-256 values are recorded in `summary.json`.
- Verification: CUDA CTest `27/27`, CPU CTest `15/15`, focused exporter/CLI Python `5 passed`, full CPU-build Python `375 passed, 126 skipped`, raw JSON parsing/hash checks, Python/C++ DSA/expert-set comparison, and `git diff --check` passed. Two earlier Python-suite failures were caused by pointing CPU-unavailable contract tests at the CUDA build directory; the standard CPU-build run passed.
- Decision: accept this as a bounded complete-layer BF16 numerical gate. Route-order bitwise parity, persistent incremental state, production batched execution, multi-layer final logits, and end-to-end tok/s remain open.

## 2026-08-16 -- B-0008 resident BF16 dense-hit fast path

- Commit/hardware: implementation commit `3b7a35a`; AMD Ryzen 7 9800X3D, NVIDIA GeForce RTX 5080 16 GB, WSL2 Ubuntu-24.04, CUDA 13. No cloud or paid resource was used.
- Model/mode: official GLM-5.2 layer 10 through the same complete DSA/MLA/residual/natural-Top-8 routed-plus-shared MoE boundary as B-0007. Precision was BF16-rounded with FP32 output, 2 GiB resident budget, and 64 MiB cuBLAS workspace.
- Change: resident raw-BF16 dense weights are looked up before converting the complete FP32 host view. First admission retains the original conversion/validation path. The CUDA regression proves that the second resident call adds zero host-weight conversion calls/bytes, exact repeated output, and no warm weight H2D.
- Historical two-token control: B-0007 commit `902f551`, three warmups and seven iterations, measured `475,927,299 ns` median (`237.9636495 ms/token`, `4.202322691 layer-tokens/s`).
- Optimized two-token result: ten warmups and 100 iterations measured `5,503,443 ns` median (`2.7517215 ms/token`, `363.408869684 layer-tokens/s`). This is an `86.4781x` bounded improvement and `98.8436%` latency reduction. Warm weight H2D remained zero, and expected-output absolute/relative error remained `0.015625` / `0.00420168088749`.
- Optimized single-token result: ten warmups and 100 iterations measured `2,756,654 ns` (`2.756654 ms/layer-token`, `362.758619689 layer-tokens/s`). Warm weight H2D was zero; expected-output absolute/relative error was `0.015625` / `0.00434782588854`.
- Traffic/residency: the single-token row performed 43 logical source reads for `1,031,436,800` bytes during setup, moved `679,477,248` cold weight bytes, and retained `1,028,259,840` resident bytes. The two-token row performed 67 reads for `1,635,416,576` bytes, moved `1,283,457,024` cold bytes, and retained `1,632,239,616` bytes. Both warm paths moved zero weight bytes.
- Profile interpretation: after the fix, approximately `87.9%` of aggregate GPU kernel time in the bounded profile belonged to BF16 GEMV kernels; CUDA stream synchronization was small. Host conversion is no longer the dominant warm path.
- Derived bound, not measurement: multiplying the one-token layer by 78 yields `215.019012 ms/token` or `4.650751534 tok/s` before final norm, logits, storage, runtime control, and heterogeneous-layer effects. It is an optimistic lower-time/upper-throughput extrapolation, not full-model TPS.
- Full-model metrics: decode tok/s, prefill tok/s, TTFT, process peak VRAM, system RAM peak, physical NVMe GB/token, coding quality, and speculative acceptance were not measured. The best exact full-model decode remains `0.010559 tok/s`; 10 tok/s remains unachieved.
- Artifacts: `results/b0008-resident-bf16-fast-path-rtx5080/{optimized.json,single-token.json,summary.json,summary.csv}`. Raw SHA-256 values are `28838e6de06bcc4bd1d26a5e78bfcede0ce9fe75726ccc73590a51c38cee97ec` and `f23a409779fc642c99f5bf3e2a7e24a93d5f688bdab96cc1b7309af749202983`.
- Verification: CUDA CTest `27/27`, CPU CTest `15/15`, focused Python `5 passed`, JSON/CSV/hash/parity checks, and `git diff --check` passed. The full Python suite was not repeated for this C++-only optimization; the prior B-0007 full run remains `375 passed, 126 skipped`.
- Decision: accept the exact resident-hit optimization. The next architecture decision must target BF16 weight traffic with calibrated native FP4 or a comparably large packed/fused resident projection path, then prove multi-layer final-logit and quality parity.

## 2026-08-16 -- B-0009 resident W8A16-G128 routed experts

- Date/commit: 2026-08-16; implementation commit `51ca9bc` on `codex/index-backed-cuda-moe`.
- Hardware/environment: AMD Ryzen 7 9800X3D, NVIDIA GeForce RTX 5080 16 GB, 96 GB host RAM, WSL2 Ubuntu-24.04, CUDA 13. No cloud or paid resource was used.
- Model/mode/context: official GLM-5.2 layer 10 through input RMSNorm, full DSA/MLA attention, both residuals, natural sigmoid Top-8 routed MoE plus the BF16 shared expert, and full-layer output. Routed gate/up/down weights used signed W8A16-G128 with BF16 scales; activations, trunk, shared expert, and FP32 output accumulation remained BF16-compatible. Resident budget was 2 GiB and cuBLAS workspace was 64 MiB.
- Two-token result: ten warmups and 100 iterations measured `4,520,669 ns` median (`2.2603345 ms/token`, `442.4124 layer-tokens/s`). B-0008 BF16 measured `5,503,443 ns` under the same token/warmup/iteration boundary, so bounded latency fell `17.857%` (`1.2174x`).
- Single-token result: ten warmups and 100 iterations measured `2,223,067 ns` (`2.223067 ms/layer-token`, `449.8290 layer-tokens/s`). B-0008 BF16 measured `2,756,654 ns`, so bounded latency fell `19.356%` (`1.2400x`).
- Traffic/residency: the two-token routed payload was `613,416,960` bytes versus `1,207,959,552` BF16 bytes, a `49.21875%` reduction. Cold weight H2D fell from `1,283,457,024` to `688,914,432` bytes; warm weight H2D remained zero. Total resident bytes were `1,037,697,024`. The single-token W8 routed payload was `306,708,480` bytes and total resident bytes were `730,988,544`.
- Correctness/quality: DSA Top-K, natural route IDs, and route contributions matched the BF16 control exactly. Complete-layer GPU/CPU relative L2 was `0.0002294650` for two tokens and `0.0002330593` for one token. Expected-output maximum relative error remained `0.0042016809` and `0.0043478259`, respectively.
- Kernel interpretation: Nsight's bounded W8 projection sample sustained roughly `825--837 GB/s` of effective weight traffic, so the routed kernels are already memory-bound. The remaining layer wall time is dominated by the attention/trunk host-round-trip and projection sequence; a projection-grouping experiment regressed and was removed.
- Derived bound, not measurement: multiplying the one-token layer result by 78 yields `173.399226 ms/token` or `5.767038 tok/s` before final norm, logits, storage, runtime control, and heterogeneous-layer effects. This is not full-model decode TPS and does not establish the 10 tok/s target.
- Full-model metrics not measured: decode/prefill tok/s, TTFT, process peak VRAM, system RAM peak, physical NVMe GB/token, full-model H2D GB/token, coding quality, and speculative acceptance. The best exact measured full-model decode remains `0.010559 tok/s`; 10 tok/s remains unachieved.
- Artifacts: `results/b0009-w8a16-expert-runtime-rtx5080/{two-token.json,single-token.json,summary.json,summary.csv}`. Raw SHA-256 values are `1cb619bc0c144a49866912786d42953ed0bb01e1b332f03e304a1090fdbe3ac8` and `3a8e605a571812f450f3a17ff4ffe84ef4131b92f38463456d6d8ac1e2473f3f`.
- Verification: CUDA CTest `29/29`, CPU CTest `16/16`, focused Python contract `6 passed`, JSON/hash/formula parity, and `git diff --check` passed. Persistent `.pw8` sidecars and multi-layer final-logit parity remain open.
- Decision: accept W8A16-G128 only as an experimental bounded routed-expert runtime candidate. Keep BF16 fallback and default behavior unchanged; do not build the full packed checkpoint until persistent artifact and multi-layer quality gates exist.
